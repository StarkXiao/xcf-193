  ...
  switch (type) {
    ...
    case 'world_entry': {
      for (const world of worldSettingsData) {
        ...
        if (entry) return `设定-${entry.title}`;
      }
      return '未知设定条目';
    }
    case 'comment': {
...
router.post('/:type/:id/takedown', (req, res) => {
  const { type, id } = req.params;
  ...
  const log = addAuditLog(type, id, 'takedown', null, operatorId || 'admin-1', operatorName || '管理员', reason);
  updateAuditStats();
  ...
  if (authorId) {
    ...
    createNotification({
      userId: authorId,
      type: 'audit_rejected',
      content: `你的${typeLabel}《${targetTitle}》已被下架：${reason}`,
      relatedId: id,
      relatedType: type,
      relatedTitle: targetTitle,
      extra: {
...
router.post('/:type/:id/restore', (req, res) => {
  const { type, id } = req.params;
  const { auditLevel, operatorId, operatorName } = req.body;
  ...
  switch (type) {
    case 'story': {
      target = storiesData.find(s => s.id === id);
      if (target) {
        target.auditStatus = auditLevel ? 'approved' : 'approved';
        target.auditLevel = auditLevel || 'G';
        target.isTakenDown = false;
        target.rejectReason = undefined;
      }
      break;
    }
    ...
    case 'comment': {
      for (const storyId in commentsData) {
        ...
        if (comment) {
          target = comment;
          comment.auditStatus = 'approved';
          comment.auditLevel = auditLevel || 'G';
          comment.isTakenDown = false;
          comment.rejectReason = undefined;
          break;
        }
      }
  ...
  const log = addAuditLog(type, id, 'restore', auditLevel || 'G', operatorId || 'admin-1', operatorName || '管理员', '内容已恢复上架');
  updateAuditStats();
    ...
    storiesData
      ...
      .forEach(s => {
        allItems.push({
          ...
          authorId: s.authorId,
          authorName: s.authorName,
          takedownReason: s.rejectReason,
          takenDownAt: s.updatedAt
        });
      });
  }
    ...
    worldSettingsData
      ...
      .forEach(w => {
        allItems.push({
          ...
          authorId: w.authorId,
          authorName: w.authorName,
          takedownReason: w.rejectReason,
          takenDownAt: w.updatedAt
        });
      });
  }
